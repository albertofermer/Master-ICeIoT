...
class LayerGradCam(LayerAttribution, GradientAttribution):
    ...
    def attribute(
        ...
        target_average_gradients = list()
        for target, target_weight in zip(target_list, target_weights):
            ...
            target_average_gradients.append(summed_grads)
        
        average_gradients = (torch.stack([t[0] for t in target_average_gradients], dim=0).sum(dim=0),)

        scaled_acts = tuple(
            torch.sum(average_gradient * layer_eval, dim=1, keepdim=True)
            for average_gradient, layer_eval in zip(average_gradients, layer_evals)
        )
