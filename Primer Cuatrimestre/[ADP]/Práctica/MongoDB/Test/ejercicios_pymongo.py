...
#client = pymongo.MongoClient("mongodb+srv://gcerruela:XXXXXXXXXXX@cluster0.5m0pk.mongodb.net/sample_db?retryWrites=true&w=majority")
...
if "weekly_demand" in collist:
  ...
  weekly_demand_collection = database.get_collection("weekly_demand")
      ...
      with open("weekly_demand.json") as f:
        ...
        records = list(weekly_demand_collection.find())
        num_records = len(records)
        #print("Collection weekly_demand created: " + str(weekly_demand_collection.find().count()) + 'registros');
        print("Collection weekly_demand created: " + str(num_records) + ' registros');
...
if "meal_info" in collist:
  ...
  meal_info_collection = database.get_collection("meal_info")
      ...
      with open("meal_info.json") as f:
          ...
          records = list(meal_info_collection.find())
          num_records = len(records)
          print("Collection meal_info created: " + str(num_records)  + ' registros');
